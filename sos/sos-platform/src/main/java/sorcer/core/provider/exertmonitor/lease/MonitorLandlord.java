...
public class MonitorLandlord implements Landlord, Runnable, ReferentUuid, Remote {
	...
	BasicJeriExporter exporter = new BasicJeriExporter(TcpServerEndpoint.getInstance(0),
							   new BasicILFactory());
	...
	if (resource != null) {
	    ...
	    //       " next lease duration="+ (expiration - System.currentTimeMillis()));
	...
	logger.log(Level.INFO,"leases renewed Landlord.RenewResults="+results);
        return results;
    }
	...
	for (Enumeration e=resources.keys(); e.hasMoreElements(); ) {
		...
		logger.log(Level.INFO,"Lease cancelled for resource ="+resource);
		resource.leaseCancelled();
		resources.remove(resource.getCookie());
	    } else if (resource.getTimeout() < now) {
		logger.log(Level.INFO,"Timeout for resource ="+resource+ " resource.getTimeout()="+resource.getTimeout()+" now="+now+" resource.getTimeout()-now="+(resource.getTimeout()-now));
		resource.timedOut();
		resources.remove(resource.getCookie());
	    }
	}
