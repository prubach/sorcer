...
public class ControlContext extends ServiceContext<Object> implements Strategy, IControlContext {
	...
	/*public void setQosSpacerRepeatTimes(Integer times) {
		if (times != null)
			put(QOSSPACER_REPEAT_TIMES, times);
	}

	public Integer getQosSpacerRepeatTimes() {
		return (Integer) get(QOSSPACER_REPEAT_TIMES);
	}

	public void setQosSpacerTimeout(Long timeout) {
		if (timeout != null)
			put(QOSSPACER_TIMEOUT, timeout);
	}

	public Long getQosSpacerTimeout() {
		return (Long) get(QOSSPACER_TIMEOUT);
	}

	public void setQosSpacerMinProviders(Integer prov) {
		if (prov != null)
			put(QOSSPACER_MIN_PROVIDERS, prov);
	}

	public Integer getQosSpacerMinProviders() {
		return (Integer) get(QOSSPACER_MIN_PROVIDERS);
	}
