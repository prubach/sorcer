...
cd "%SORCER_HOME%\bin"
CALL java %JAVA_OPTS% -classpath "%BROWSER_CLASSPATH%" -Dssb.logFile=..\configs\browser\logs\browser.log -Dssb.logLen=300 -Djava.protocol.handler.pkgs="net.jini.url|sorcer.util.url|org.rioproject.url" -Djava.security.policy="%SORCER_HOME%\configs\browser\policy\ssb.policy" -Dwebster.internal="true" -Dwebster.tmp.dir="%SORCER_HOME%\data" -Dprogram.name=Browser %STARTER_MAIN_CLASS% %CONFIG% %*
popd
