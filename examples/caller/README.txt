...
3) start the service itself by calling
    sorcer-boot :caller-cfg
4) test the service by running its requestor:
    ant -f caller-req/run.xml
5) You can also run the netlet script by invoking:
    nsh -f run.ntl
    On UNIX you can also simply run the script ./run.ntl
6) You can also run the generated integration test located in: caller-req/src/test/java/
   This test will automatically start the provider and run the test on it - to run it go to caller-req
   and run:
   mvn -Prun-int test
