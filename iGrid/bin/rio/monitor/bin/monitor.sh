...
$JAVA_HOME/bin/java -server $classpath -Djava.security.policy=$RIO_HOME/policy/policy.all -Djava.protocol.handler.pkgs=net.jini.url -DRIO_HOME=$RIO_HOME -DJINI_HOME=$JINI_HOME -DRIO_LOG_DIR=$RIO_LOG_DIR $launchTarget $RIO_HOME/configs/start-monitor.config
