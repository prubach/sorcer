...
"%JAVA_HOME%\bin\java" -hotspot %classpath% -Djava.security.policy=%RIO_HOME%\policy\policy.all -Djava.protocol.handler.pkgs=net.jini.url -DRIO_HOME=%RIO_HOME% -DJINI_HOME=%JINI_HOME% -DRIO_LOG_DIR=%RIO_LOG_DIR% %launchTarget% %SORCER_HOME%\bin\rio\cybernode\configs\start-cybernode.config
