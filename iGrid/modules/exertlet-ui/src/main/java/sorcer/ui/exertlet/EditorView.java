...
public class EditorView extends JPanel implements HyperlinkListener {
	...
	private final JFileChooser fileChooser = new JFileChooser(
			System.getProperty("iGrid.home"));
	private JIconButton homeButton;
	private JButton editButton, saveButton, openButton, saveAsButton,
			exertButton;
	...
	public EditorView(String input, boolean withLocator, boolean editable,
		...
		this(input, withLocator, editable, withEditing, isEditor, isDisposable,
				null);
	}
	...
	private ExertionThread runTaskScript(String script) {
		String serviceType = model.getSelectedInterfaceName();
		String selector = model.getSelectedMethod();
		StringBuilder sb = new StringBuilder();
		sb.append(staticImports.toString()).append("\nimport ")
				.append(serviceType).append(";\n").append("task(sig(\"")
				.append(selector).append("\",").append(serviceType)
...
//						.displayContextClassLoaderTree();
	...
	private void setDebug() {
		...
		if (debugProperty != null
				&& (debugProperty.toLowerCase().equals("true") || debugProperty
