...
package sorcer.falcon.examples.requestor.fnEval.legacy.fnEvalWhileTask;
...
public class FnEvalWhileTask {
	...
	public void executeTask() {
		...
		WhileExertion whileTask = new WhileExertion(task);
		...
		try {
			...
			// the
			// condition
			whileTask.setConditionVariable("X2", "in/value/var2");
			whileTask.setConditionVariable("result", ((FnContext) task
					.getContext()).getResultPath()); // here path is already set
			// with in the FnContext
			whileTask.setCondition("result < 10000"); // set the condition for
			// the loop
			...
			log.info("Condition: " + "\"result < 10000\"");
