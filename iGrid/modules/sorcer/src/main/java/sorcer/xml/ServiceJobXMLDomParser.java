...
import java.util.*;
import sorcer.core.*;
import sorcer.core.provider.ServiceTask;
...
public class ServiceJobXMLDomParser {
    ...
    */
	...
	if (name.equals("ServiceJob")) {
	    // add all the tasks
	    for (Enumeration e = allTasks.elements(); e.hasMoreElements() ;) {
		job.addTask((ServiceTask)e.nextElement());
	    }
	    allTasks.clear();
	} else if (name.equals("ServiceTask")) {
	    // add all the task contexts
	    for (Enumeration e = allTaskContexts.elements(); e.hasMoreElements() ;) {
		task.addContext((ServiceContext)e.nextElement());
	    }
	    allTasks.add(task);
