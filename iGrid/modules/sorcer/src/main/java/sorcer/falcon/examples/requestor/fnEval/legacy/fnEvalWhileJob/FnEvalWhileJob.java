...
package sorcer.falcon.examples.requestor.fnEval.legacy.fnEvalWhileJob;
...
public class FnEvalWhileJob {
	...
	public void executeTask() {
		...
		// looping a job
		WhileExertion whileTask = new WhileExertion(job);
		...
		try {
			...
			// the MasterExertion is the last task/job unless explicitly
			// specified
			whileTask.setConditionVariable("result", ((FnContext) ((Job) job)
					.getMasterExertion().getContext()).getResultPath()); // here
			// path
			// is
			// already
			// set
			...
			// the
			// FnContext
			whileTask.setCondition("result < 500"); // set the condition for the
			// loop
			...
			log.info("Where: x = " + resultContext.get("in/value/var1")
					+ "  y = " + resultContext.get("in/value/var2"));
			log.info("Condition: \"result < 500\"");
	...
	public Job createFnJob() {
		...
		job.setMasterExertion(task2); // explicitly specify the master exertion
