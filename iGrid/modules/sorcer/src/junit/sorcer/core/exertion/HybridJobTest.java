...
import sorcer.arithmetic.provider.Multiply;
...
//		Var<?> x3 = var("x3", expr("x3-e", "x1 - x2", vars("x1", "x2")));
...
//
//		Task t5 = task("t5",
//				sig(expr("x2 + x3", vars(var("x2", 20.0), var("x3", 80.0)))),
...
//		//ObjectJob
//		Job job = job("j1", sig("execute", ExertionJobber.class), 
//				job(sig("execute", ExertionJobber.class), "j2", t4, t5), t3,
//				pipe(out(t4, path(result, y)), in(t3, path(arg, x1))),
//				pipe(out(t5, path(result, y)), in(t3, path(arg, x2))));
//		
//		// NetJob
