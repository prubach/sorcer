...
import sorcer.core.context.model.ParametricModel.Strategy;
...
import sorcer.vfe.ModelingAdmin;
import sorcer.vfe.OptimizationModelingAdmin;
import sorcer.vfe.ParametricModeling;
import sorcer.vfe.ParametricModelingAdmin;
 ...
 *
 * This command is used for inspecting Var-oriented models. It allows for:
 * 1. Inspecting both network (vm) and local (vm -init <model builder>) var models,
 *  for example: vm -init junit.sorcer.core.context.model.RosenSuzukiModelBuilder.createModel(); 
 * 2. Evaluating (vm -e {var names}) a collection of observed (selected model input and output) vars
 * 3. Setting parametric (a collection of selected model input) vars (vm -s {var names})
 * 4. Calculating response tables (vm -rt <parametric table URL>), for example
 *  using geometry/parametric_datafile_long.txt in ${IGRID_HOME}/data
 */
public class VarModelCmd extends ShellCmd {
		...
		if (next.equals("-p")) {
			...
			if (numTokens == 2) {
				...
				if (next.equals("-rt")) {
					...
					if (!(ptUrl.startsWith("http://") || ptUrl.startsWith("sos://"))) {
			...
			if (numTokens == 2) {
				...
				if (next.startsWith("-")) {
					if (next.equals("-pt")) {
						next = tokenizer.nextToken();
						if (!(next.startsWith("http://") || next.startsWith("sos://"))) {
	...
	static private void showModelService(int index, String msg) throws RemoteException {
		out.println("---------" + (msg != null ? " " + msg : "")
				+ " VAR MODEL SERVICE # " + index + " ---------");
		out.println("MOD: " + modelProviders[index].serviceID + " at: "
				+ AttributesUtil.getHostName(modelProviders[index].attributeSets));
		out.println("Home: "
				+ AttributesUtil.getUserDir(modelProviders[index].attributeSets));
		String groups = AttributesUtil.getGroups(modelProviders[index].attributeSets);
		out.println("Provider name: "
				+ AttributesUtil.getProviderName(modelProviders[index].attributeSets));
		out.println("Groups supported: " + groups);
		try {
			// cast used to test for the existence of modeling admin implementation
	...
	private int selectModelByName(String name) {
		if (modelProviders != null) {
			for (int i = 0; i < modelProviders.length; i++) {
				if (AttributesUtil.getProviderName(modelProviders[i].attributeSets)
						.equals(name))
					return i;
			}
		}
	...
	private void printInputVarsDependencies()
			throws RemoteException, ContextException {
		ModelingAdmin modelAdmin = (ModelingAdmin) getModelingAdmin();
		out.println(modelAdmin.printInputVarsDependencies());
	}
	
	private void printOutputVarsDependencies()
			throws RemoteException, ContextException {
		ModelingAdmin modelAdmin = (ModelingAdmin) getModelingAdmin();
		out.println(modelAdmin.printOutputVarsDependencies());
	}

	private void printConstraintVarsDependencies()
			throws RemoteException, ContextException {
		ModelingAdmin modelAdmin = (ModelingAdmin) getModelingAdmin();
		out.println(modelAdmin.printConstraintVarsDependencies());
	}

	private void printObjectiveVarsDependencies()
			throws RemoteException, ContextException {
		ModelingAdmin modelAdmin = (ModelingAdmin) getModelingAdmin();
	...
	private void printModelDependencies() throws RemoteException,
			ContextException {
		ModelingAdmin modelAdmin = (ModelingAdmin) getModelingAdmin();
		out.println("Var Dependency for "
	...
	private void describeConstraintVars() throws RemoteException, ContextException {
		ModelingAdmin modelAdmin = (ModelingAdmin) getModelingAdmin();
	...
	private void describeObjectiveVars() throws RemoteException, ContextException {
		ModelingAdmin modelAdmin = (ModelingAdmin) getModelingAdmin();
	...
	private String printVarInfo(String varName) throws RemoteException,
			EvaluationException, ContextException {
		ModelingAdmin modelAdmin = getParametricModelingAdmin();
		return "" + modelAdmin.getVarInfo(varName);
	}
	...
	private List<String> getRemainingTokens(String first, StringTokenizer tokenizer) {
	...
	private void listCurrentState() throws RemoteException {
		...
		if (model == null) {
			ModelingAdmin modelAdmin = (ModelingAdmin) getModelingAdmin();
			if (modelAdmin != null)
				out.println("Current Var Model: " + modelAdmin.getProviderName());
		} else {
	...
	private void printParametricTable() throws RemoteException,
		...
		if (modelAdmin != null)
			table = modelAdmin.getParametricTable();
		if (table == null) {
	...
	private void printResponseTable() throws RemoteException, ContextException {
		...
		ParametricModelingAdmin modelAdmin = getParametricModelingAdmin();
		if (modelAdmin != null)
			table = modelAdmin.getResponseTable();
		if (table == null) {
	...
	private void printModelStrategy() throws RemoteException {
		...
		if (admin instanceof ParametricModelingAdmin) {
			...
			Strategy strategy = modelAdmin.getStrategy();
			if (strategy == null) {
	...
	private void evaluateResponseTable() throws RemoteException, EvaluationException {
		ParametricModeling pmodel = getParametricModeling();
		((Modeling)pmodel).evaluate();
		Table table = ((ParametricModeling)pmodel).getResponseTable();
	...
	private void setResponseTable(URL responseTableUrl)
			throws RemoteException, EvaluationException {
		ParametricModeling pmodel = getParametricModeling();
		pmodel.setResponseTableURL(responseTableUrl);
	}

	private void evaluateResponseTable(URL parametricTableUrl) throws RemoteException, EvaluationException {
		ParametricModeling pmodel = getParametricModeling();
		pmodel.resetParametricTableURL(parametricTableUrl);
		pmodel.evaluate();
		Table table = ((ParametricModeling)pmodel).getResponseTable();
	...
	public static void printCurrentModel() throws RemoteException {
		if (modelIndex >= 0) {
			...
			NetworkShell.shellOutput.println("No selected Var Model, use 'vm' command");
		}
	}
