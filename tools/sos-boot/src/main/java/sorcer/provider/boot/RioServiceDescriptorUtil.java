...
public final class RioServiceDescriptorUtil {
     ...
     */
    private RioServiceDescriptorUtil() {
    }

    static int getStartupPort() throws IOException {
        if(port==0) {
            if(System.getProperty(PORT_RANGE)!=null) {
                port = PortUtil.getPortFromRange(System.getProperty(PORT_RANGE));
            } else {
    ...
    public static ServiceDescriptor getCybernode(final String policy, final String... cybernodeConfig) throws IOException {
        ...
        String cybernodeCodebase = "artifact:org.rioproject.cybernode/cybernode-proxy/"+ SorcerEnv.getRioVersion();
    ...
    public static String getCybernodeClasspath() throws IOException {
        ...
        if(System.getProperty("RIO_TEST_ATTACH")!=null) {
            System.setProperty(RESOLVER_JAR, getProjectResolverLocation(rioHome));
