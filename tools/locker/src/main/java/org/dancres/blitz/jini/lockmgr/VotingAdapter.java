...
public class VotingAdapter implements MessageListener, MembershipListener {
	...
	throws ChannelException
    {
        ...
        if (log.isLoggable(Level.FINER)) log.fine("Conducting voting on decree " + decree + ", consensus type " +
	...
	default : mode = GroupRequest.GET_ALL;
        ...
        try {
            ...
            if (log.isLoggable(Level.FINER)) log.fine("Calling remote methods...");
    
            // vote       
            RspList responses = rpcDispatcher.callRemoteMethods(null, methodCall,  
            		new RequestOptions().setMode(mode).setTimeout(timeout));
            
            if (log.isLoggable(Level.FINER)) log.fine("Checking responses.");
            ...
            if (log.isLoggable(Level.FINER)) log.fine("Could not find method localVote(Object). " +
			nsmex.toString());
    ...
    private int checkResponse(Rsp response) throws ChannelException {
        ...
        if (!response.wasReceived()) {
        	if (log.isLoggable(Level.FINER)) log.fine("Response from node " + response.getSender() +
			    " was not received.");
        ...
        if (response.wasSuspected()) {
                ...
                if (log.isLoggable(Level.FINER)) log.fine("Node " + response.getSender() + " was suspected.");
        ...
        if (object instanceof FailureVoteResult) {
        	...
        	if (log.isLoggable(Level.FINER)) log.severe(((FailureVoteResult)object).getReason());
    ...
    public VoteResult localVote(Object decree) {
        ...
        for(int i = 0; i < listeners.length; i++) {
            ...
            try {
            	...
            	if (log.isLoggable(Level.FINER)) log.severe(ex.toString());
        ...
        if (log.isLoggable(Level.FINER)) log.fine("Voting on decree " + decree.toString() + " : " +
			voteResult.toString());
