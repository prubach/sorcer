...
public class AdminView {
	...
	private static boolean IS_IX = true;

	static {
		try {
			Class.forName("org.cheiron.jsc.ServiceState");
			IS_IX = false;
	...
	private JDesktopPane getRuntimeIfPanel(Entry[] atts, LookupLocator[] locs,
		...
		// boolean isLifeCycle=adminProxy instanceof
		// org.cheiron.jsc.admin.LifeCycleAdmin;
	...
	public static boolean canDestroy(Object adminProxy) {
		...
		if (!IS_IX) {
			if (adminProxy instanceof org.cheiron.jsc.admin.LifeCycleAdmin) {
				return true;
			}
		}
	 ...
	 * 
	 * final org.cheiron.jsc.admin.LifeCycleAdmin lcAdmin
	 * =(org.cheiron.jsc.admin.LifeCycleAdmin)_adminProxy;
	 * 
					...
					public void run() {
						try {
							if (IS_IX) {
								...
								if (_adminProxy instanceof org.cheiron.jsc.admin.LifeCycleAdmin) {
									// System.out.println("destroy() [org.cheiron.jsc.admin.LifeCycleAdmin] "+_adminProxy);
									org.cheiron.jsc.admin.LifeCycleAdmin destroyAdmin = (org.cheiron.jsc.admin.LifeCycleAdmin) _adminProxy;
									destroyAdmin.destroy();
								} else {
									DestroyAdmin destroyAdmin = (DestroyAdmin) _adminProxy;
									destroyAdmin.destroy();
